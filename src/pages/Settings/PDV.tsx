...
import { Copy, Eye, EyeOff, Plus, Power, Store, Trash2 } from 'lucide-react';
import { useCreatePDVMutation, useGetPDVsQuery, useDeletePDVMutation, useTogglePDVStatusMutation } from '@/features/pdvApiSlice';
...
const PDV = () => {
  ...
  const [toggleStatus] = useTogglePDVStatusMutation();
  ...
  const onSubmit = async (values: z.infer<typeof formSchema>) => {
    try {
      await createPDV({ ...values, tenantId }).unwrap();
  ...
  const handleDeletePDV = async (id: string) => {
    try {
      ...
      toast({
        title: 'Erro',
        description: 'Erro ao remover PDV. Tente novamente.',
        variant: 'destructive',
      });
    }
  };
                  ...
                  <TableRow key={pdv.id}>
                    ...
                    <TableCell className='text-right'>
                      <div className='flex justify-end gap-2'>
                        <Button variant='ghost' size='icon' onClick={() => handleToggleStatus(pdv.id)}>
                          <Power className='w-4 h-4' />
                        </Button>
                        <Button variant='ghost' size='icon' onClick={() => handleDeletePDV(pdv.id)}>
